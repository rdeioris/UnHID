...
public:
	...
	void SetVitualInputDeviceAxis(const int32 ControllerId, const FName& Axis, const float Value)
	{
...
void FUnHIDModule::SetVitualInputDeviceAxis(const int32 ControllerId, const uint8 AxisId, const float Value)
{
	if (AxisId > 127)
	{
		return;
	}
	UnHIDInputDevice->SetVitualInputDeviceAxis(ControllerId, AxisCache[AxisId], Value);
}
